#Add option for spectral clustering & option for kmeans sweep. Also use networkX graphs to separate objects that have been improperly combined
...
class GaborFilters:
    ...
    class SetOfGroups:
        ...
        def updateGroupArea(self):
            self.groupArea = (self.maxRow - self.minRow) * (self.maxCol - self.minCol)
        ...
        def splitGroups(self):
            ...
            plt.show()
            """
            #calculate whether 90% of all degree values fall below 60% of the number of groups in the array. If so, remove all groups that have degree values above 60% of the number of groups in the array
            nodeDegreesSorted = np.sort(nodeDegrees,axis=0)
            sixtyPercentOfGroupNum = len(self.groups)*0.6
            ...
            if nodeDegrees90thPercentile < sixtyPercentOfGroupNum:
                for i in range(len(nodeDegrees)):
                    if nodeDegrees[i] > len(self.groups)*0.6:
                        G.remove_node(i)

            #calculate communities
            
            communities = community.best_partition(G, resolution = 0.01)
            ...
            #fold communities that are less than 70% of the size of the largest community into the largest community
            indexOfGroupsToRemove = []
            for i in range(len(communitySizes)):
                if communitySizes[i] < 0.7 * maxCommunitySize:
                    indexOfGroupsToRemove.append(i)
            ...
            for i in range(len(groupsArr)):
                if i in communities:
                    if not communities[i] == maxCommunitySizeIndex:
                        if not communities[i] in indexOfGroupsToRemove:
                            if communities[i] in newSetOfGroups:
                                newSetOfGroups[communities[i]].addGroup(self.groups[i-numberOfGroupsDeletedFromSelf])
                            else:
                                newSetOfGroups[communities[i]] = self.__class__([self.groups[i-numberOfGroupsDeletedFromSelf]], self.groupSizeHeight, self.groupSizeWidth, self.numAlternations)
    ...
    class ObjectHistory:
        ...
        def predictedPositionAfterNotBeingSeen(self):
            return np.array([self.positionHistory[0]+self.vectorHistory[0]*self.numFramesWithoutBeingSeen, self.positionHistory[1]+self.vectorHistory[1]*self.numFramesWithoutBeingSeen, self.positionHistory[2]+self.vectorHistory[0]*self.numFramesWithoutBeingSeen, self.positionHistory[3]+self.vectorHistory[1]*self.numFramesWithoutBeingSeen])
    ...
    def matchGroupsAndDeterminePeakFilter(self,grayHistory):
        ...
        for nextFrameROI in self.nextFrameROIs:
            ...
            for combinedGroup in combinedGroups:
                ...
                counter+=1
            for combinedGroup in combinedGroups:
                combinedGroup.updateGroupArea()
            sorted(combinedGroups, key = lambda SetOfGroup: SetOfGroup.groupArea)
            for combinedGroup in combinedGroups:
