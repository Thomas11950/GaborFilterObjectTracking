            ...
            for j in range(int(4), int(w-4)):
                if mat[i][j][0] > self.gaborThreshold:
                    ...
                    if groupNum == -1:
                        ...
                        sumPeaksAfterThreshold.append(mat[i][j][0]-self.gaborThreshold)
                        nextGroupNumber += 1
                    else:
                        mat[i][j][1] = groupNum
                        ...
                        sumPeaksAfterThreshold[groupNum] += (mat[i][j][0]-self.gaborThreshold)
        groups = []
        for i in range(nextGroupNumber):
            groups.append([[(groupMinRow[i]+groupMaxRow[i])/2, (groupMinCol[i]+groupMaxCol[i])/2],[groupMinRow[i], groupMinCol[i], groupMaxRow[i], groupMaxCol[i]],sumPeaksAfterThreshold[i]])
        return groups
