#avg gabor instead of max gabor
            ...
            for j in range(int(4), int(w-4)):
                if mat[i][j][0] > self.gaborThreshold:
                    ...
                    if groupNum == -1:
                        ...
                        sumGabor[groupNum] += mat[i][j][0]
        groups = []
        for i in range(nextGroupNumber): 
            if groupSize[i] > 1:
                groups.append([[(groupMinRow[i]+groupMaxRow[i])/2, (groupMinCol[i]+groupMaxCol[i])/2],[groupMinRow[i], groupMinCol[i], groupMaxRow[i], groupMaxCol[i]],[sumGabor[i]/groupSize[i]*self.gaborBaseVector[0], sumGabor[i]/groupSize[i]*self.gaborBaseVector[1]]])
        return groups
...
class GaborFilters:
    ...
    numGaborSpeeds = 4;
    numGaborAngles = 8;
    def __init__(self, gaborThreshold, groupSizeHeight, groupSizeWidth, baseVectorSize, numAlternations):
        self.gaborFilters = []
    ...
    def getGroupsFromDifferentFilters(self,grayHistory):
        ...
        for i in range(numGaborSpeeds):
            for j in range(numGaborAngles):
                for k in range(self.numAlternations):

                    groupsToAdd = self.gaborFilters[i][j].getGroups(grayHistory[int(len(grayHistory)/2)-3*(int(k)+1):int(len(grayHistory)/2)+3*(int(k)+1)+1:int(k)+1])
                    for group in groupsToAdd:
                        group[2][0] *= (k+1)
                        group[2][1] *= (k+1)
                    groupList.append(groupsToAdd)
                    print("\nfinished convolving with filter " + str(i+1) + "_" + str(j+1) + "_" + str(k+1) + "\n")
        return groupList
    ...
    def matchGroupsAndDeterminePeakFilter(self,grayHistory):
        ...
        combinedGroups = []
        for i in range(numGaborSpeeds*numGaborAngles*self.numAlternations):
...
numAlternations = int(input("Num Alternations: "))
frameHistorySizeNeeded = numAlternations*6+1
# Norfair
...
gaborFilters = GaborFilters(gaborThreshold,groupSizeHeight,groupSizeWidth, 10, numAlternations)
for frame in video:
