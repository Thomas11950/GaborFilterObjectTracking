...
class GaborFilters:
    ...
    class ROI:
        ...
        def getIDs(self, groups):
            ...
            overlaps = np.zeros((len(groups), len(self.predictedBoundingBoxesForContainedIDs)),dtype = "float")
            for i in range(len(groups)):
                for j in range(len(self.predictedBoundingBoxesForContainedIDs)):
                    overlaps[i][j] = getRectangularOverlap(groups[i].determineGroupBoundingBox(), self.predictedBoundingBoxesForContainedIDs[j])
            
            while not np.amax(overlaps) == 0:
                rowOfMax, colOfMax = np.unravel_index(overlaps.argmax(), overlaps.shape)
                dictGroupIndicesToID[groups[rowOfMax]] = self.containedIDs[colOfMax]
                for i in range(len(self.predictedBoundingBoxesForContainedIDs)):
                    overlaps[rowOfMax][i] = 0
                for i in range(len(groups)):
                    overlaps[i][colOfMax] = 0
