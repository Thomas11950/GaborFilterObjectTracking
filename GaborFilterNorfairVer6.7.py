...
class GaborFilters:
    ...
    class ROI:
        ...
        def getIDs(self, groups):
            ...
            if len(self.predictedBoundingBoxesForContainedIDs) <= len(groups):
                predictedBoundingBoxesForContainedIDsFirstPriority = self.predictedBoundingBoxesForContainedIDs
                predictedBoundingBoxesForContainedIDsSecondPriority = []
            else:
                predictedBoundingBoxesForContainedIDsFirstPriority = self.predictedBoundingBoxesForContainedIDs[0:len(groups):1]
                predictedBoundingBoxesForContainedIDsSecondPriority = self.predictedBoundingBoxesForContainedIDs[len(groups):len(self.predictedBoundingBoxesForContainedIDs):1]
            ...
            print("containedIDs: " + str(self.containedIDs))
            ...
            if not len(predictedBoundingBoxesForContainedIDsSecondPriority) == 0:
                while not np.amax(overlapValuesSecondPriority) == 0:
                    ...
                    dictGroupIndicesToID[rowOfMax] = self.containedIDs[colOfMax+len(groups)]
                    for i in range(len(predictedBoundingBoxesForContainedIDsSecondPriority)):
                        overlapValuesSecondPriority[rowOfMax][i] = 0
                    for i in range(len(groups)):
            ...
            if len([key for key, values in rev_multidict.items() if len(values) > 1]) > 0:
                print("exit now, but before that:\noverlap values first priority: " + str(overlapValuesFirstPriority) + "\noverlap values second priority: " + str(overlapValuesSecondPriority) +"\ndict: " + str(dictGroupIndicesToID)+"\n")
                exit()
            return dictGroupIndicesToID
    ...
    def matchGroupsAndDeterminePeakFilter(self,grayHistory):
        ...
        for nextFrameROI in self.nextFrameROIs:
            ...
            print("used IDs: " + str(self.usedIDs)+"\n")
            ...
            for combinedGroup in superCombinedGroups:
                ...
                for objectHistory in self.objectHistoriesForGivenID:
                    if objectHistory.getID() == combinedGroup.getGroupSetID():
                        objectHistoryWithCorrectID = objectHistory
