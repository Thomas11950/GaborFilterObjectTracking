#DBSCAN implementation for grouping
...
class GaborFilters:
    ...
    numGaborSpeeds = 4;
    numGaborAngles = 8;
    def __init__(self, gaborThreshold, baseVectorSize, numAlternationsMin, numAlternationsMax, alternationInterval):
        self.gaborFilters = []
        for i in range(numGaborSpeeds):
            ...
            for j in range(numGaborAngles):
                if i == 0:
                    ...
                    movementMagnitude = -2
                listToAdd.append(GaborFilter("Filters/filter_"+str(i+1)+"_"+str(j+1)+".mat", gaborThreshold, [-baseVectorSize * movementMagnitude * np.sin(j * 0.3926991 - np.pi/2),baseVectorSize * movementMagnitude * np.cos(j * 0.3926991 - np.pi/2)]))
...
gaborFilters = GaborFilters(gaborThreshold, 3, numAlternationsMin, numAlternationsMax, alternationInterval)
for frame in video:
