#Scan entire frame every set number of frames to detect new objects
...
class GaborFilters:
    ...
    global sizeOfObjectSizeHistory
    global checkEntireFrameInterval
    numGaborSpeeds = 4
    numGaborAngles = 8
    sizeOfObjectSizeHistory = 100
    ...
    def __init__(self, gaborThreshold, baseVectorSize, numAlternationsMin, numAlternationsMax, alternationInterval, numCoresAvailable, objectExpansionBuffer):
        ...
        self.objectSizeHistoriesForGivenID = []
        self.usedIDs = []
    ...
    class ObjectSizeHistory:
        def __init__(self, ID, initialObjectSize):
            self.ID = ID
            self.sizeHistory = np.full((sizeOfObjectSizeHistory,2),initialObjectSize, dtype = "float")
        def getID(self):
            return self.ID
        def addToSizeHistory(self, sizeToAdd):
            for i in range(sizeOfObjectSizeHistory-1):
                self.sizeHistory[sizeOfObjectSizeHistory-1-i] = self.sizeHistory[sizeOfObjectSizeHistory-2-i]
        ...
        def getAvgSizeAcrossHistory(self):
            ...
            for size in self.sizeHistory:
                sumHeight += size[0]
                sumWidth += size[1]
            return np.array([sumHeight/sizeOfObjectSizeHistory, sumWidth/sizeOfObjectSizeHistory])
    ...
    def matchGroupsAndDeterminePeakFilter(self,grayHistory):
        ...
        if self.countMatchGroupsAndDeterminePeakFilter % checkEntireFrameInterval == 0:
            ...
            for combinedGroup in superCombinedGroups:
                ...
                objectFound = False
                for objectSizeHistoryForGivenID in self.objectSizeHistoriesForGivenID:
                    if objectSizeHistoryForGivenID.getID() == combinedGroup.getGroupSetID():
                        objectFound = True
                        objectSizeHistoryForGivenID.addToSizeHistory(np.array(combinedGroup.determineGroupSize()))
                        maxObjectSizeInHistory = objectSizeHistoryForGivenID.getAvgSizeAcrossHistory()
                if not objectFound:
                    self.objectSizeHistoriesForGivenID.append(self.ObjectSizeHistory(combinedGroup.getGroupSetID(),np.array(combinedGroup.determineGroupSize())))
...
gaborFilters = GaborFilters(gaborThreshold, 0.01, numAlternationsMin, numAlternationsMax, alternationInterval, numCores, 0.4)
for frame in video:
