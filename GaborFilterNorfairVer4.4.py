...
import cv2
import numpy as np
from multiprocessing import Process as Processs, Queue
...
def returnGroupsFromGaborFilters(gaborFilters, groupsQueue):
...
class GaborFilters:
    ...
    numGaborSpeeds = 4;
    numGaborAngles = 8;
    def __init__(self, gaborThreshold, groupSizeHeight, groupSizeWidth, baseVectorSize, numAlternationsMin, numAlternationsMax, alternationInterval, numCoresAvailable):
        self.gaborFilters = []
        for i in range(numGaborSpeeds):
            ...
            for j in range(numGaborAngles):
                ...
                for k in range(numAlternationsMin,numAlternationsMax+1, alternationInterval):
                    subListToAdd.append(GaborFilter("Filters/filter_"+str(i+1)+"_"+str(j+1)+".mat", gaborThreshold,groupSizeHeight, groupSizeWidth, [-baseVectorSize * k * movementMagnitude * np.sin(j * 0.3926991 - np.pi/2),baseVectorSize * k* movementMagnitude * np.cos(j * 0.3926991 - np.pi/2)]))
    ...
    def getGroupsFromDifferentFilters(self,grayHistory):
        ...
        for i in range(self.numCoresAvailable):
            ProcessList.append(Processs(target=returnGroupsFromGaborFilters,args=(gaborFilters[int(i*numCoresPerFilter):int((i+1)*numCoresPerFilter):1], groupsQueue)))
...
gaborFilters = GaborFilters(gaborThreshold,groupSizeHeight,groupSizeWidth, 3, numAlternationsMin, numAlternationsMax, alternationInterval, numCores)
for frame in video:
