...
class GaborFilter:
    global minPts
    minPts = 4
    def __init__(self, GaborFilterFilepath, gaborThreshold, gaborBaseVector):
    ...
    def getGroups(self):
        ...
        for i in range(numClusters): 
            groups.append([[(groupMinRows[i]+groupMaxRows[i])/2, (groupMinCols[i]+groupMaxCols[i])/2],[groupMinRows[i], groupMinCols[i], groupMaxRows[i], groupMaxCols[i]],[maxGabors[i]*self.gaborBaseVector[0], maxGabors[i]*self.gaborBaseVector[1]]])
...
class GaborFilters:
    ...
    numGaborSpeeds = 4;
    numGaborAngles = 8;
    def __init__(self, gaborThreshold, baseVectorSize, numAlternationsMin, numAlternationsMax, alternationInterval, numCoresAvailable):
        self.gaborFilters = []
        for i in range(numGaborSpeeds):
            ...
            for j in range(numGaborAngles):
                ...
                for k in range(numAlternationsMin,numAlternationsMax+1, alternationInterval):
                    subListToAdd.append(GaborFilter("Filters/filter_"+str(i+1)+"_"+str(j+1)+".mat", gaborThreshold, [-baseVectorSize * k * movementMagnitude * np.sin(j * 0.3926991 - np.pi/2),baseVectorSize * k* movementMagnitude * np.cos(j * 0.3926991 - np.pi/2)]))
    ...
    def matchGroupsAndDeterminePeakFilter(self,grayHistory):
        ...
        groupsFromDifferentFiltersData = self.getGroupsFromDifferentFilters(grayHistory)
        ...
        for i in range(numGaborSpeeds*numGaborAngles*int((self.numAlternationsMax-self.numAlternationsMin)/self.alternationInterval+1)):
            for groupToAdd in groupsFromDifferentFilters[i]:
                ...
                if not addedIntoExistingSet:
                    combinedGroups.append(self.SetOfGroups([groupToAdd], groupSizeHeight, groupSizeWidth))
...
gaborFilters = GaborFilters(gaborThreshold, 3, numAlternationsMin, numAlternationsMax, alternationInterval, numCores)
for frame in video:
