#objects are temporary before they have established histories. Temporary objects are labelled with NEGATIVE NUMBERS
...
class GaborFilters:
    ...
    global numFramesNecessaryToMakeObjectPermanent
    numFramesNecessaryToMakeObjectPermanent = 10
    numGaborSpeeds = 4
    numGaborAngles = 8
    ...
    class SetOfGroups:
        def __init__(self, groups, groupSizeHeight, groupSizeWidth, numAlternations):
            ...
            self.minCol = boundingGroup[1]
            self.maxRow = boundingGroup[2]
        ...
        def splitGroups(self):
            ...
            communities = community.best_partition(G, resolution = 0.0005)
            ...
            #fold communities that are less than 50% of the size of the largest community into the community that has the most cuts
            communitiesToRemove = []
            communitiesToKeep = []
            for i in range(len(communitySizes)):
            ...
            for communityToRemove in communitiesToRemove:
                communityToKeepWithHighestCutSize = maxCommunitySizeIndex
                highestCutSize = 0
                for communityToKeep in communitiesToKeep:
                    ...
                    if cut_Size > highestCutSize:
                        communityToKeepWithHighestCutSize = communityToKeep
                        highestCutSize = cut_Size
                if communityToKeepWithHighestCutSize == maxCommunitySizeIndex:
                    self.addAnotherSetOfGroups(newSetsOfGroups[communityToRemove])
                else:
                    newSetsOfGroups[communityToKeepWithHighestCutSize].addAnotherSetOfGroups(newSetsOfGroups[communityToRemove])
    ...
    class ROI:
        def __init__(self, ROIposition, containedIDs, predictedBoundingBoxesForContainedIDs):
            ...
            self.dictContainedIDsToPredictedBoundingBoxes = {}
            index = 0
            for containedID in containedIDs:
                self.dictContainedIDsToPredictedBoundingBoxes[containedID] = predictedBoundingBoxesForContainedIDs[index]
                index+=1
        def __str__(self):
            return str(self.ROIposition)
        def addObjectToROI(self, ID, boundingBoxForContainedID):
            self.dictContainedIDsToPredictedBoundingBoxes[ID] = boundingBoxForContainedID
        def getPos(self):
            return self.ROIposition
        def getContainedIDs(self):
            return self.dictContainedIDsToPredictedBoundingBoxes.keys()
        def getPredictedBoundingBoxesForContainedIDs(self):
            return self.dictContainedIDsToPredictedBoundingBoxes.values()
        def getIDs(self, groups):
            ...
            self.dictContainedIDsToPredictedBoundingBoxes = dict(sorted(self.dictContainedIDsToPredictedBoundingBoxes.items()))
            predictedBoundingBoxesForContainedIDs = self.dictContainedIDsToPredictedBoundingBoxes.values()
            containedIDs = self.dictContainedIDsToPredictedBoundingBoxes.keys()
            dictGroupIndicesToID = {}
            if len(groups) == 0 or len(predictedBoundingBoxesForContainedIDs) == 0:
                return dictGroupIndicesToID
            predictedBoundingBoxesForContainedIDsFirstPriority = []
            predictedBoundingBoxesForContainedIDsSecondPriority = []
            #find the first positive ID in the dict
            firstPositiveIDIndex = len(containedIDs)
            for i in range(len(containedIDs)):
                if containedIDs[i] > 0:
                    firstPositiveIDIndex = i 
                    break
            predictedBoundingBoxesForContainedIDsSecondPriority = predictedBoundingBoxesForContainedIDs[0:firstPositiveIDIndex:1]
            predictedBoundingBoxesForContainedIDsFirstPriority = predictedBoundingBoxesForContainedIDs[firstPositiveIDIndex:len(containedIDs):1]
            
            overlapValuesFirstPriority = np.zeros((len(groups), len(predictedBoundingBoxesForContainedIDsFirstPriority)),dtype = "float")
            for i in range(len(groups)):
                for j in range(len(predictedBoundingBoxesForContainedIDsFirstPriority)):
                    overlapValuesFirstPriority[i][j] = getRectangularOverlap(groups[i].determineGroupBoundingBox(), predictedBoundingBoxesForContainedIDsFirstPriority[j])
                    
            
            overlapValuesSecondPriority = np.zeros((len(groups), len(predictedBoundingBoxesForContainedIDsSecondPriority)),dtype = "float")
            for i in range(len(groups)):
                for j in range(len(predictedBoundingBoxesForContainedIDsSecondPriority)):
                    overlapValuesSecondPriority[i][j] = getRectangularOverlap(groups[i].determineGroupBoundingBox(), predictedBoundingBoxesForContainedIDsSecondPriority[j])
            

            while not np.amax(overlapValuesFirstPriority) == 0:
                rowOfMax, colOfMax = np.unravel_index(overlapValuesFirstPriority.argmax(), overlapValuesFirstPriority.shape)
                    
                dictGroupIndicesToID[rowOfMax] = containedIDs[colOfMax]
                for i in range(len(predictedBoundingBoxesForContainedIDsFirstPriority)):
                    overlapValuesFirstPriority[rowOfMax][i] = 0
                for i in range(len(predictedBoundingBoxesForContainedIDsSecondPriority)):
                    overlapValuesSecondPriority[rowOfMax][i] = 0
                for i in range(len(groups)):
                    overlapValuesFirstPriority[i][colOfMax] = 0
                print("overlap values first priority: " + str(overlapValuesFirstPriority) + "\noverlap values second priority: " + str(overlapValuesSecondPriority) +"\ndict: " + str(dictGroupIndicesToID)+"\n")
                print("rowOfMax: " + str(rowOfMax)+", colOfMax: " + str(colOfMax)+"\n")
            
            
            if not len(predictedBoundingBoxesForContainedIDsSecondPriority) == 0:
                while not np.amax(overlapValuesSecondPriority) == 0:
                    rowOfMax, colOfMax = np.unravel_index(overlapValuesSecondPriority.argmax(), overlapValuesSecondPriority.shape)
                        
                    dictGroupIndicesToID[rowOfMax] = containedIDs[colOfMax+len(groups)]
                    for i in range(len(predictedBoundingBoxesForContainedIDsSecondPriority)):
                        overlapValuesSecondPriority[rowOfMax][i] = 0
                    for i in range(len(groups)):
                        overlapValuesSecondPriority[i][colOfMax] = 0
                    print("overlap values first priority: " + str(overlapValuesFirstPriority) + "\noverlap values second priority: " + str(overlapValuesSecondPriority) +"\ndict: " + str(dictGroupIndicesToID)+"\n")
                    print("rowOfMax: " + str(rowOfMax)+", colOfMax: " + str(colOfMax)+"\n")
            
            rev_multidict = {}
            for key, value in dictGroupIndicesToID.items():
    ...
    class ObjectHistory:
        ...
        def resetNumFramesWithoutBeingSeen(self):
            self.numFramesWithoutBeingSeen = 0
        def incrementNumFramesInExistence(self):
            self.numFramesInExistence+=1
        def getNumFramesInExistence(self):
            return self.getNumFramesInExistence
        def predictedPositionAfterNotBeingSeen(self):
            return self.predictedPositionAfterNumFrames(self.numFramesWithoutBeingSeen)
        def predictedPositionAfterNumFrames(self, numFrames):
            return np.array([self.positionHistory[0][0]+self.vectorHistory[0][0]*numFrames, self.positionHistory[0][1]+self.vectorHistory[0][1]*numFrames, self.positionHistory[0][2]+self.vectorHistory[0][0]*numFrames, self.positionHistory[0][3]+self.vectorHistory[0][1]*numFrames])
    ...
    def matchGroupsAndDeterminePeakFilter(self,grayHistory):
        ...
        for nextFrameROI in self.nextFrameROIs:
            ...
            for i in range(len(combinedGroups)):
                if not i in groupSetIDs:
                    objectFoundFromHistory = False
                    for objectHistoryForGivenID in self.objectHistoriesForGivenID.values():
                        if getRectangularOverlap(objectHistoryForGivenID.predictedPositionAfterNotBeingSeen(), combinedGroups[i].determineGroupBoundingBox()) > 0.5 and not objectHistoryForGivenID.getID() in groupSetIDs.values():
                            objectFoundFromHistory = True
                            groupSetID = objectHistoryForGivenID.getID()
                            break
                    if not objectFoundFromHistory:
                        j = -1
                        while True:
                            if not j in self.usedTempIDs:
                                groupSetID = j
                                self.usedTempIDs.append(j)
                                break
                            j-=1
        ...
        for combinedGroup in superCombinedGroups:
            if combinedGroup.getGroupSetID() in self.objectHistoriesForGivenID:
                if combinedGroup.getGroupSetID() < 0 and self.objectHistoriesForGivenID[combinedGroup.getGroupSetID()].getNumFramesInExistence() > numFramesNecessaryToMakeObjectPermanent:
                    j = 1
                    while True:
                        if not j in self.usedIDs:
                            oldTempID = combinedGroup.getGroupSetID()
                            combinedGroup.setGroupSetID(j)
                            self.objectHistoriesForGivenID[j] = self.objectHistoriesForGivenID[oldTempID]
                            self.objectHistoriesForGivenID[j].setID(j)
                            self.usedTempIDs.remove(oldTempID)
                            self.usedIDs.append(j)
                            break
                        j+=1

        for ID in self.objectHistoriesForGivenID:
            objectInHistorySeen = False
            for combinedGroup in superCombinedGroups:
                if ID == combinedGroup.getGroupSetID():
                    objectInHistorySeen = True
            if not objectInHistorySeen:
                self.objectHistoriesForGivenID[ID].incrementNumFramesWithoutBeingSeen()
            else:
                self.objectHistoriesForGivenID[ID].resetNumFramesWithoutBeingSeen()
        ...
        if self.countMatchGroupsAndDeterminePeakFilter % checkEntireFrameInterval == 0:
            ...
            for combinedGroup in superCombinedGroups:
                ...
                objectHistoryWithCorrectID = self.objectHistoriesForGivenID[combinedGroup.getGroupSetID()]
                objectHistoryAvgSize = objectHistoryWithCorrectID.getAvgSizeAcrossHistory()
                ...
                if sizeIsSignificantDeviationFromHistoricalSize:
                    combinedGroupPeakGaborNextPredictedBoundingBoxPosition = objectHistoryWithCorrectID.predictedPositionAfterNumFrames(1)
                else:
                    combinedGroupPeakGaborNextPredictedBoundingBoxPosition = [combinedGroupOnlyPeakGabor[0][0]+combinedGroupOnlyPeakGabor[1][0],combinedGroupOnlyPeakGabor[0][1]+combinedGroupOnlyPeakGabor[1][1],combinedGroupOnlyPeakGabor[0][2]+combinedGroupOnlyPeakGabor[1][0],combinedGroupOnlyPeakGabor[0][3]+combinedGroupOnlyPeakGabor[1][1]]
                
                if combinedGroup.getGroupSetID() in self.objectHistoriesForGivenID:
                    self.objectHistoriesForGivenID[combinedGroup.getGroupSetID()].addToSizeAndPositionAndVectorHistory(np.array(combinedGroup.determineGroupSize()),np.array(combinedGroupOnlyPeakGabor[0]),np.array(combinedGroupOnlyPeakGabor[1]))
                else:
                    self.objectHistoriesForGivenID[combinedGroup.getGroupSetID()] = self.ObjectHistory(combinedGroup.getGroupSetID(),np.array(combinedGroup.determineGroupSize()),np.array(combinedGroupOnlyPeakGabor[0]),np.array(combinedGroupOnlyPeakGabor[1]))
            ...
            for combinedGroup in superCombinedGroups:
                ...
                maxObjectSizeInHistory = self.objectHistoriesForGivenID[combinedGroup.getGroupSetID()].getAvgSizeAcrossHistory()
